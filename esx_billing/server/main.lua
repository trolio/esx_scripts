		...
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					...
					TriggerClientEvent('esx:showNotification', xTarget, _U('received_invoice'))
				end)
			else
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					['@identifier'] = xTarget.identifier,
			...
			if result[1].target_type == 'player' then
				if xTarget then
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								TriggerClientEvent('esx:showNotification', xPlayer, _U('paid_invoice', ESX.Math.GroupDigits(amount)))
								TriggerClientEvent('esx:showNotification', xTarget, _U('received_payment', ESX.Math.GroupDigits(amount)))
							end
					...
					elseif xPlayer.getAccount('bank').money >= amount then
						...
						TriggerClientEvent('esx:showNotification', xTarget, _U('target_no_money'))
						TriggerClientEvent('esx:showNotification', xPlayer, _U('no_money'))
					...
					TriggerClientEvent('esx:showNotification', xPlayer, _U('player_not_online'))
				...
				TriggerEvent('esx_addonaccount:getSharedAccount', result[1].target, function(account)
					...
					elseif xPlayer.getAccount('bank').money >= amount then
						...
						--	xTarget.showNotification(_U('target_no_money'))
							...
							TriggerClientEvent('esx:showNotification', xTarget, _U('no_money'))
						--xPlayer.showNotification(_U('no_money'))
						cb()
					end
				end)
