...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			...
			TriggerClientEvent('esx:showNotification', _source, _U('have_deposited', ESX.Math.GroupDigits(amount)))
		else
			TriggerClientEvent('esx:showNotification', _source, _U('invalid_amount'))
		end
	else
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('esx:showNotification', _source, _U('you_have_been_hired', job))
			elseif type == 'promote' then
				TriggerClientEvent('esx:showNotification', _source, _U('you_have_been_promoted'))
			elseif type == 'fire' then
				TriggerClientEvent('esx:showNotification', _source, _U('you_have_been_fired', xTarget.getJob().label))
			end
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				TriggerClientEvent('esx:showNotification', _source, _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)))
			end
